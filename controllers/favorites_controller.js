...
async function removeItemFromFavorite(req, res) {
    try {
        ...
        if (productID) {
            const userFavorite = await FavoriteModel.findOne({ consumerID: consumerID });
            if (userFavorite) {
                const index = userFavorite.favoriteProducts.findIndex(p => p.productID === productID);
                if (index != -1) {
...
module.exports = { addToFavorite, removeItemFromFavorite, getFavorites, clearFavorites };
